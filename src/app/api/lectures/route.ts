...
export async function POST(req: NextRequest) {
  ...
  if (typeof subject !== 'string' || !subject.trim() || subject.length > 200) {
    return NextResponse.json({ error: 'Subject must be a non-empty string with maximum 200 characters' }, { status: 400 });
  }
  if (typeof content !== 'string' || !content.trim() || content.length > 200000) {
    return NextResponse.json({ error: 'Content must be a non-empty string with maximum 200,000 characters' }, { status: 400 });
  }
  const lectures = readJSON<Lecture[]>('lectures.json', []);
